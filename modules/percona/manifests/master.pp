...
class percona::master inherits percona {
  ...
  exec { 'init user sstuser':
    command => "mysql --defaults-extra-file=/root/.my.cnf -u root -e \"CREATE USER \'${percona::wsrep_sst_user}\'@\'localhost\' IDENTIFIED BY \'${percona::wsrep_sst_password}\';\"",
    path    => [ '/bin', '/usr/bin' ],
    unless  => "test -f ${percona::datadir}/first/db.opt",
    require => Exec[$percona::percona_service]
  } ->
  exec { 'grant privileges':
    command => "mysql --defaults-extra-file=/root/.my.cnf -u root -e \"GRANT RELOAD, LOCK TABLES, REPLICATION CLIENT ON *.* TO \'${percona::wsrep_sst_user}\'@\'localhost\';\"",
    path    => [ '/bin', '/usr/bin' ],
    unless  => "test -f ${percona::datadir}/first/db.opt",
    require => Exec[$percona::percona_service],
  } ~>
  exec { 'grant process':
    command => "mysql --defaults-extra-file=/root/.my.cnf -u root -e \"GRANT USAGE ON *.* TO 'clustercheckuser'@'localhost' IDENTIFIED BY 'CLUSTERCHECK_PWD'; FLUSH PRIVILEGES;\"",
    path    => [ '/bin', '/usr/bin' ],
    unless  => "test -f ${percona::datadir}/first/db.opt",
    require => Exec[$percona::percona_service],
  } ~>
  exec { 'create a firt database':
    command => "mysql --defaults-extra-file=/root/.my.cnf -u root -e \"CREATE DATABASE first\"",
    path    => [ '/bin', '/usr/bin' ],
    unless  => "test -f ${percona::datadir}/first/db.opt",
    require => Exec[$percona::percona_service],
  }
